...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def _check_valid_request_line(self, request_line_ids):
        ...
        for line in self.env['purchase.request.line'].browse(request_line_ids):
            ...
            if picking_type is not False \
                ...
                line_location = picking_type.default_location_dest_id or False
    ...
    def default_get(self, fields):
        ...
        res['item_ids'] = items
        supplier_ids = request_line_obj.browse(request_line_ids).mapped('supplier_id').ids
        if len(supplier_ids) == 1:
            res['supplier_id'] = supplier_ids[0]
        return res

    # @api.model
    # def get_line_item_value_function(self):
    # for rec in self:
    # request_line_obj = self.env['purchase.request.line']
    #         request_line_ids = self.env.context.get('active_ids', False)
    #         items = []
