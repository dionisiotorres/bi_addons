...
class SalaryAdvancePayment(models.Model):
    ...
    def _get_default_emp_account(self):
        return int(self.env['ir.config_parameter'].sudo().get_param('bi_salary_advance.sa_emp_account_id')) or False

    def _get_default_treasury_account(self):
        return int(self.env['ir.config_parameter'].sudo().get_param('bi_salary_advance.sa_treasury_account_id')) or False
    ...
    debit = fields.Many2one('account.account', string='Treasury Account', default=_get_default_treasury_account)
    ...
    def onchange_employee_id(self):
        ...
        return {
            'value': {'department': department_id, 'employee_contract_id': self.employee_id.contract_id},
    ...
    def unlink(self):
        for rec in self:
            if rec.state in ['approve']:
                raise ValidationError(_('You cannot delete approved salary advance requests.'))
...
class HrEmployee(models.Model):
    ...
    @api.one
    def _compute_employee_sa(self):
        self.sa_count = self.env['salary.advance'].search_count([('employee_id', '=', self.id)])

    sa_count = fields.Integer(string="Salary Advance Count", compute='_compute_employee_sa')
